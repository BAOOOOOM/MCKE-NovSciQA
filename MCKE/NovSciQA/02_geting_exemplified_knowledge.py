...
def get_cot_without_abs_ans(question,option,correct_answer,lecture):
    ...
    prompt3="Option: "+option+"\n"
    prompt4="Correct Answer: "+correct_answer+"\n"
    prompt5="Lecture: "+lecture+"\n"
    prompt6="Identify the plan for answering the question and give the rationale for answering the question (where the rationale should be given point by point). Note please give the full rationale (e.g. don't say based on what is mentioned in the lecture).\n"
...
def get_cot_with_example_ans(question,option,correct_answer,lecture,example):
    prompt1="You are a teacher and you need to give the rationale for answering the question in response to the information related to the question below.\n"
    ...
    prompt5="Lecture: "+lecture+"\n"
    prompt6="Identify the plan for answering the question and give the rationale for answering the question (where the rationale should be given point by point). Note please give the full rationale (e.g. don't say based on what is mentioned in the lecture).\n"
    input_text=prompt1+prompt_example+prompt2+prompt3+prompt4+prompt5+prompt6
...
def get_cot_with_example(question,option,example):
    ...
    prompt_example="Example: "+example+"\n"
    prompt2="Question: "+question+"\n"
    prompt3="Options: "+option+"\n"
    prompt4="Identify the plan for answering the question and then give the rationale for answering the question, please give the rationale directly and do not output anything else.\n"
    input_text=prompt1+prompt_example+prompt2+prompt3+prompt4
...
def get_answer_base_on_cot(question,option,rationale):
    prompt1="Below there is information related to the question, select the answer.\n"
    prompt2="Question: "+question+"\n"
    prompt3="Option: "+option+"\n"
    prompt4="Rationale: "+rationale+"\n"
    prompt5="Please give only the answer choice (e.g. (F) ) and do not output anything else (e.g. None of the above and (F) good). You must give one of [ (A), (B), (C), (D), (E) ]. The answer choice is:\n"
    input_text=prompt1+prompt2+prompt3+prompt4+prompt5
    messages = [{"role": "user", "content": input_text}]
    prompt = pipeline.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
    terminators = [pipeline.tokenizer.eos_token_id, pipeline.tokenizer.convert_tokens_to_ids("<|eot_id|>")]
    outputs = pipeline(prompt, eos_token_id=terminators, do_sample=False)
...
for skill,current_id_list in skill_example_dict.items():
    ...
    for current_id in sampled_k_id_list:
        ...
        new_pred_solution=get_cot_without_abs_ans(question=question,option=option,correct_answer=correct_answer,lecture=lecture)

        the_value="Question: "+question+" [SEP] Options: "+option+" [SEP] Rationale: "+new_pred_solution
...
for skill,current_id_list in skill_example_dict.items():
    ...
    for abs_id in selected_id_list:
        ...
        for test_id in now_test_id_list:
            ...
            new_pred_solution=get_cot_with_example(question=question,option=option,example=current_example)
            llama_answer=get_answer_base_on_cot(question=question,option=option,rationale=new_pred_solution)
            try:
...
with open(data_path,"r") as f:
    ...
    for id in data:
        ...
        new_pred_solution=get_cot_with_example_ans(question=question,option=option,correct_answer=correct_answer,lecture=lecture,example=best_example)
        llama_answer=get_answer_base_on_cot(question=question,option=option,rationale=new_pred_solution)
        ...
        current_item={"question":question,"choices":choices,"answer":answer,"subject":subject,"category":category,"human_skill":skill,"pred_solution":new_pred_solution,"pred_answer":llama_answer,"human_lecture":lecture,"human_solution":solution,"llama_is_true":llama_is_true,"selected_example":best_example,"split":split}
